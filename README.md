...
A motorized stage moves the lens up and down along the focus axis ($Z$) in steps of 0.002 mm, measuring the sharpness score (MTF) across 5 regions: Center, Upper-Left, Upper-Right, Lower-Left, and Lower-Right.

The software determines focus using three simple values:
...
* **Pass Criterion:** If $\Delta Z \le 0.006\text{ mm}$ ($6\ \mu\text{m}$), the lens is parallel and passes inspection.
...
| **Alignment Tolerance ($\Delta Z$)** | **$\le 0.006\text{ mm}$ ($6\ \mu\text{m}$)** | Enforces clear, edge-to-edge optical sharpness |
